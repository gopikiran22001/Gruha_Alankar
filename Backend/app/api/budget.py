"""Gruha Alankara — Budget API: POST /api/budget/calculate"""

from __future__ import annotations
import asyncio, uuid
from flask import Blueprint, request
from flask_jwt_extended import get_jwt_identity, jwt_required
from app.agents.schemas import AgentTask
from app.agents.registry import agent_registry
from app.api.middleware import api_response, rate_limit
from config.constants import AgentName

budget_bp = Blueprint("budget", __name__)

@budget_bp.route("/calculate", methods=["POST"])
@jwt_required()
@rate_limit(max_requests=15, window=60)
def calculate_budget():
    """Calculate or optimize a budget."""
    user_id = get_jwt_identity()
    data = request.get_json() or {}
    agent = agent_registry.get(AgentName.BUDGET)
    if not agent:
        return api_response(status="error", message="Budget agent unavailable", status_code=503)
    task = AgentTask(
        task_id=f"budget_{uuid.uuid4().hex[:8]}",
        task_type=data.get("task_type", "estimate_budget"),
        agent_name=AgentName.BUDGET,
        parameters={
            "budget": data.get("budget", 0),
            "target_budget": data.get("target_budget", 0),
            "categories": data.get("categories", []),
            "products": data.get("products", []),
        },
        constraints={"budget": data.get("budget")},
        context=data.get("context", {}),
        metadata={"user_id": user_id},
    )
    result = asyncio.run(agent.run(task))
    return api_response(data=result.data, metadata={"duration_ms": round(result.duration_ms, 1)})
